...
import { TimerConfig } from "../src/lib/store-interface.js";
...
describe("RedisTimerStore", () => {
  ...
  describe("validate()", () => {
    ...
    it("rejects name longer than 50 characters", async () => {
      await expect(store.validate(makeConfig({ name: "A".repeat(51) }))).rejects.toThrow(/too long/);
    });
    ...
    it("uses SMEMBERS + individual GET for each timer", async () => {
      ...
      mockRedis.get
        .mockResolvedValueOnce(JSON.stringify({
          id: 1, guildId: "g", channelId: "ch", name: "T1",
          intervalMinutes: 5, maxRepeat: null, triggerCount: 0,
          startedAt: Date.now(), maxDurationMs: 7200000, startedBy: "u",
        }))
        .mockResolvedValueOnce(JSON.stringify({
          id: 2, guildId: "g", channelId: "ch", name: "T2",
          intervalMinutes: 10, maxRepeat: 3, triggerCount: 1,
          startedAt: Date.now(), maxDurationMs: 7200000, startedBy: "u",
        }));
