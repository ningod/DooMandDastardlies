...
async function handleStopButton(
  ...
  if (!timer) {
    await interaction.editReply({
      embeds: [
        buildErrorEmbed(
          "This timer has already been stopped or completed.",
        ),
      ],
    });
    return;
  ...
  if (timer.channelId !== interaction.channelId) {
    await interaction.editReply({
      embeds: [
        buildErrorEmbed(
          "This timer belongs to a different channel.",
        ),
      ],
    });
    return;
...
async function handleRestartButton(
  ...
  try {
    ...
    const message =
      err instanceof TimerParseError ? err.message : "Failed to restart timer.";
    await interaction.editReply({
