...
import type { TimerInstance } from './timer-store.js';
...
export function buildTimerTriggerEmbed(opts: {
  ...
  const countDisplay = maxRepeat !== null ? `#${triggerCount}/${maxRepeat}` : `#${triggerCount}`;

  const embed = new EmbedBuilder().setTitle(TITLE).setColor(Colors.DarkGold).setTimestamp();
 ...
 */
export function buildTimerStoppedEmbed(opts: { timerId: number; name: string }): EmbedBuilder {
...
export function buildTimerListEmbed(timers: TimerInstance[]): EmbedBuilder {
  const embed = new EmbedBuilder().setTitle(TITLE).setColor(Colors.Blurple).setTimestamp();
  ...
  for (const timer of timers) {
    ...
    const remainingReps =
      timer.maxRepeat !== null ? `${timer.maxRepeat - timer.triggerCount}` : '∞';
